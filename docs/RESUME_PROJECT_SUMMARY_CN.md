...
| 当前全量测试 | Rust 143/143；Python 297/297 | Linux 隔离测试目录；双框架组件数据已真实采集 |

机器可读结果和完整限制分别位于 `benchmarks/results`、`docs/M6_REAL_PROJECT_RESULTS.md`、`docs/M7_RUNTIME_PARITY.md`、`docs/M8_SECURITY_BENCHMARK.md`、`docs/M9_CONTEXT_BENCHMARK.md` 与 `docs/M11_COMPONENT_PARITY.md`。
...
- **迁移诊断闭环：** 基于 Python AST 构建不执行目标代码的 PyTorch API 扫描器，并以版本化 Schema 串联 MindSpore 官方映射、dtype/shape/返回结构/数值/梯度 Trace 与首个偏差诊断；在 Linux 的 PyTorch 2.6/MindSpore 2.9 环境完成 MLP、CNN、梯度和 BatchNorm 等 7 个固定组件/缺陷案例双端采集，实现 7/7 等价性分类、4/4 等价组件验证及 3/3 留出偏差类别与首错 Top-1 定位正确。
...
1. **真实训练闭环：** 在已完成的组件前向/梯度基础上加入损失、优化器一步更新和权重对齐，统计补丁采用率、验证通过率与回滚率。
...
完成组件级双框架验证后，项目已具备“有真实运行证据的迁移诊断系统”雏形；下一步应优先证明真实训练步骤和外部模型切片，而不是继续堆叠合成算子案例。
