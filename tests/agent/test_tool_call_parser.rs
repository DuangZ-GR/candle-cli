...
fn rejects_missing_id() {
    let err = parse_tool_call(r#"<tool_call>{"name":"read","input":{}}</tool_call>"#)
        .expect_err("missing id should fail");

    assert_eq!(err.to_string(), "tool call is missing string field: id");
}

#[test]
fn rejects_missing_close_tag() {
    let err = parse_tool_call(r#"<tool_call>{"id":"call-1","name":"read","input":{}}"#)
        .expect_err("missing close tag should fail");

    assert!(matches!(err, ToolCallParseError::MissingCloseTag));
}
